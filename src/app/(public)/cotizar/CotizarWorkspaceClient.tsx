...
export default function CotizarWorkspaceClient({
        ...
        try {
          ...
          setMedidas((actuales) => ({
            ...
            [item.id]: {
              ...
              calculando: false,
              fabricable: false,
              mensaje: "No pudimos validar la medida.",
            },
          }));
        } finally {
  ...
  const manejarCotizacion = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!codigoPostal || !shippingMethodId) {
      setErrorCotizacion("Ingresá un código postal con una opción de envío disponible.");
      return;
    }
    ...
    try {
      ...
      setErrorCotizacion("No se pudo enviar la cotización. Intentá nuevamente.");
    } finally {
      setEnviando(false);
    }
  };
