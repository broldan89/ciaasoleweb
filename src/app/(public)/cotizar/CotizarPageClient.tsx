...
export default function CotizarPageClient({
        ...
        try {
          ...
          if (!respuesta.ok) {
            setMedidas((actuales) => ({
              ...
              [item.id]: {
                ...
                fabricable: false,
                mensaje:
                  "No pudimos validar esta medida. Revisá los datos e intentá nuevamente.",
              },
            }));
            return;
          }
          ...
          setMedidas((actuales) => ({
            ...
            [item.id]: {
              ...
              calculado: true,
              fabricable: false,
              mensaje: "No pudimos validar la medida. Intentá nuevamente.",
            },
          }));
        } finally {
  ...
  useEffect(() => {
    if (!codigoPostal || !/^\d{4,5}$/.test(codigoPostal)) {
      setOpcionesEnvio([]);
      setShippingMethodId("");
      setShippingCost(0);
      setErrorEnvio("");
  ...
  const seleccionarEnvio = (id: string) => {
    const opcion = opcionesEnvio.find(
      (actual) => actual.shipping_method_id === id,
    );
  ...
  const manejarCotizacion = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!todosFabricables) {
      setErrorCotizacion(
        "Completá las medidas de todos los productos y esperá la validación automática antes de continuar.",
      );
      return;
    }
    ...
    <div className="cs-section py-14 sm:py-20 lg:py-24">
      <div className="max-w-4xl">
        ...
        <p className="mt-5 max-w-2xl text-base leading-7 text-[var(--cs-muted)]">
          Elegí los sistemas, definí las medidas y obtené una estimación de
          fabricación, envío y total en un mismo lugar.
        </p>
      </div>
      ...
      <section className="cs-card mt-10 p-6 sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-md text-sm leading-6 text-[var(--cs-muted)]">
            Los productos agregados aparecen inmediatamente debajo para
            continuar con la configuración del proyecto.
          </p>
        </div>
        ...
        {catalogo.length ? (
          ...
          <p className="mt-6 border-y border-[var(--cs-line)] py-6 text-sm text-[var(--cs-muted)]">
                  ...
                  return (
                    <article key={item.id} className="px-6 py-7 sm:px-8">
                      ...
                      <div className="mt-4 border-t border-[var(--cs-line)] pt-4">
                        {estado.calculando ? (
                          <p className="text-xs text-[var(--cs-muted)]">
                            Verificando disponibilidad de fabricación...
                          </p>
                        ) : estado.calculado ? (
                          <div
                          ...
                          <p className="text-xs text-[var(--cs-muted)]">
                            La validación se realiza automáticamente al
                  ...
                  {opcionesEnvio.map((opcion) => (
                    ...
                    >
                      <input
                        ...
                        checked={
                          shippingMethodId === opcion.shipping_method_id
                        }
                        onChange={() =>
                          seleccionarEnvio(opcion.shipping_method_id)
                        }
                        className="mt-1"
                      />
